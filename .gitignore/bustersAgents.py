...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            # get the highest probability for the current ghost
            maxProbability = max([ghost[pos] for pos in ghost])
            ghostPosition = [pos for pos in ghost if ghost[pos] == maxProbability]
            ghostPosition = ghostPosition[0]
            # if we have no chosen ghost initialize with current ghost
            if closestGhost is None:
                closestGhost = ghostPosition
            currentDistance = self.distancer.getDistance(pacmanPosition, closestGhost)
            nextDistance = self.distancer.getDistance(pacmanPosition, ghostPosition)
            # if the current ghost is closer to us then the the previous closest, then update the closest
            if nextDistance < currentDistance:
                closestGhost = ghostPosition

        chosenDirection = None
        closestDistance = self.distancer.getDistance(pacmanPosition, closestGhost)
        ...
        for action in legal:
            ...
            # if the distance from the next position is smaller then the current distance
            # means that we are getting closer to the ghost
            if self.distancer.getDistance(nextPosition, closestGhost) < closestDistance:
                return action
