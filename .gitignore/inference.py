...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # emissionModel[d] - Probability of ghost to be at distance d
        ...
        # This is clearly stated in initial comments, can't explain more
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                if prob > 0:
                    # The probability of a ghost to be at p is the probability 
                    # of the gost to be at distance trueDistance | the ghost was 
                    # at p in the last step
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            # pretend the ghost is at p, and see where it can move and with what
            # probabilities
            newGhostState = self.setGhostPosition(gameState, p)
            newPosDist = self.getPositionDistribution(newGhostState)
            for newPos, prob in newPosDist.items():
                # the probability to be at initial p, and then to move to newPos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [ state for state in self.legalPositions for i in range(self.numParticles)]
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # just add an arbitrary value for each particle and normalize in the end
        dist = util.Counter()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # use itertools to make a cartesian product between legalPositions
        permutations = [p for p in itertools.product(self.legalPositions, repeat=self.numGhosts)]
        # this is stated to get them as random as possible
        random.shuffle(permutations)
        # this just appends the list to itself multiple times to fit the number of particles
        permutations = permutations * (self.numParticles // len(permutations) + 1)
        # the number is not exact tho so we should need to cut some of the leftovers
        self.particles = permutations[0 : self.numParticles-1]
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for jailed in jailedGhosts:
                newParticle = self.getParticleWithGhostInJail(newParticle, jailed)
            ...
            for ghost in range(self.numGhosts):
                if ghost not in jailedGhosts:
                    ...
                    trueDistance = util.manhattanDistance(pacmanPosition, newParticle[ghost])
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            # this is pretty much the same as elapse time without joint
            # just make the ghost move one random valid step
            for ghost in range(self.numGhosts):
